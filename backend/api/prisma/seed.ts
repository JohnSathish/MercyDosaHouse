...
async function main() {
  ...
  const categories = [
    ...
    { name: 'Idly', slug: 'idly', sortOrder: 2, icon: '🥘', badge: 'VEG' as const },
  ...
  for (const cat of categories) {
    const status = (cat as { status?: string }).status ?? 'PUBLISHED';
  ...
  await prisma.deliveryConfig.upsert({
    ...
    update: {
      ...
      minOrderAmount: 100,
      message: 'Pickup Orders Only — Home Delivery Is Not Available.',
      expansionMessage: 'Home Delivery Coming soon to all areas of Tura!',
      isActive: true,
    },
    create: {
      id: deliveryConfigId,
  ...
  // Delivery executives
  const deliveryRole = await prisma.role.findUnique({ where: { name: UserRole.DELIVERY_STAFF } });
  const riders = [
    {
      ...
      for (let i = 0; i < 40; i++) {
        ...
        await prisma.auditLog.create({
          data: {
            ...
            severity: ['INFO', 'SUCCESS', 'WARNING'][i % 3] as 'INFO' | 'SUCCESS' | 'WARNING',
